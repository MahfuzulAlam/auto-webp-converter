...
Images are almost always the heaviest part of a web page. A typical photo uploaded from a phone or camera is 2–5 MB. Multiply that by every image on every page, and your visitors are downloading a lot of unnecessary data.

Slow pages cost you real results:

- **Visitors leave.** Most people abandon a page that takes more than 3 seconds to load.
- **Google ranks you lower.** Page speed and Core Web Vitals are official ranking factors — heavy images directly hurt your SEO.
- **You pay more for hosting.** Bigger files mean more storage and more bandwidth on every single visit.

### The solution: WebP — smaller images, same quality
...
| **Faster pages** | Images are 25–35% smaller on average — pages load noticeably quicker. |
...
**What it does:** Controls how much your images are compressed, on a scale of 0–100. Higher numbers = better quality but bigger files; lower numbers = smaller files but more visible compression.
...
| --- | --- | --- |
| Blog or business site | **80** (default) | Great quality, big savings — nobody will notice the difference. |
...
**Default:** JPEG and PNG (the two most common formats — this covers 95% of typical uploads).
...
| **GIF (non-animated)** | You upload static GIF images | **Animated GIFs are automatically detected and never converted** — your animations are always safe. |
...
**Default:** ⬜ Off — only the WebP file is kept (maximum space savings).

**When enabled:** uploading `photo.jpg` gives you **two items in your Media Library**:

- `photo.webp` — the optimized version, used everywhere on your site
- `photo.jpg` — your untouched original, available whenever you need it
...
> This section only appears when the [Directorist](https://wordpress.org/plugins/directorist/) directory plugin is active. The plugin also automatically teaches Directorist to accept `.webp` files — no configuration needed for that.
...
**Example:** Your business directory gets 50 new listings a week, each with 3 photos averaging 3 MB. That's ~450 MB of images per week. With conversion enabled at quality 80, that drops to roughly 130–150 MB — saving you gigabytes of storage every month, and your listing pages stay fast.
...
- 🎞️ **Animated GIFs are never converted** — animations always survive.
...
Conversion adds a fraction of a second per image during upload. There is zero impact on your site's visitors — nothing runs on your public pages.
